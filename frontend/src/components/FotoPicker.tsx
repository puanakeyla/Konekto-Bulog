...
export default function FotoPicker({ label, file, onChange, progress, error }: Props) {
  ...
  return (
    <div>
      ...
      <div className="min-h-20 rounded-md border border-dashed border-border bg-white p-3 flex items-center gap-3">
        {preview ? (
          <img src={preview} alt={label} className="w-16 h-16 object-cover rounded flex-shrink-0" />
        ) : (
          <div className="w-16 h-16 rounded bg-surface flex flex-col items-center justify-center text-gray-400 text-[10px] text-center flex-shrink-0">
