...
export default function PoSppForm({ po, onChanged }: { po: PoItem; onChanged?: () => void }) {
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      ...
      onChanged?.()
      toast.success(`No. SPP PO ${po.no_po} terkirim. Lanjut isi Status Sergab.`)
    },
    onError: (err) => toast.error(apiErrorMessage(err, 'Gagal mengirim No. SPP.')),
  })
  ...
  return (
    <form className="po-card @container" onSubmit={(e) => { e.preventDefault(); setConfirmKirim(true) }}>
      <div className="po-card-header">
        ...
        <span className="badge badge-warning">Siap kirim</span>
      </div>
      ...
      <div className="mt-4 flex justify-end border-t border-border pt-4">
        <button type="submit" disabled={!noSpp.trim() || mutation.isPending} className="btn btn-primary">
          {mutation.isPending ? 'Mengirim...' : 'Kirim SPP ke Keuangan'}
        </button>
      </div>

      <ConfirmDialog
        open={confirmKirim}
        title="Kirim SPP ke Keuangan?"
        description={<>No. SPP akan disimpan untuk PO <strong>{po.no_po}</strong>. Pengadaan tetap berstatus draft sampai Status Sergab disimpan.</>}
        confirmLabel="Kirim SPP"
